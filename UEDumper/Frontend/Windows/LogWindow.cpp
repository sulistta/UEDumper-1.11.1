...
void windows::LogWindow::Log(logLevels level, const std::string& origin, const char* fmt, ...)
{
	...
	FILE* stream = (ENUM(level) & ENUM(logLevels::LOGLEVEL_ERROR)) ? stderr : stdout;
...
std::string windows::LogWindow::getLastLogMessage()
{
	...
	const std::string msg = logs.empty() ? std::string() : std::string(logs.back().message);
