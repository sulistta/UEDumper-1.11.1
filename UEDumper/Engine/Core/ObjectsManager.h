...
#pragma pack(pop)
	...
	{
		FFieldManager()
			: linkedFFieldIndexCount(0),
			  linkedFFieldPtrs(),
			  pFFieldArray(0),
			  maxFFieldCacheNum(FFIELD_CT),
			  linkedFFieldClassIndexCount(0),
			  linkedFFieldClassPtrs(),
			  pFFieldClassArray(0),
			  maxFFieldClassCacheNum(FFIELD_CLASSES_CT)
		{
		}

		//FField
		int linkedFFieldIndexCount;
		std::unordered_map<uint64_t, uint64_t> linkedFFieldPtrs;

		//ptr to the allocated buffer where all FFields are located
		uint64_t pFFieldArray;

		int64_t maxFFieldCacheNum;


		//FFieldClass
		int linkedFFieldClassIndexCount;
		std::unordered_map<uint64_t, uint64_t> linkedFFieldClassPtrs;

		//ptr to the allocated buffer where all FFieldClasses are located
		uint64_t pFFieldClassArray;

		int64_t maxFFieldClassCacheNum;
	};
